...
### [x] A1 — Step 2 Qualifying Checklist (PRD §5.3.2)
...
### [x] A2 — Key Meetings: attendees field (PRD §5.3.3)
...
### [x] A3 — Teaming Strategy: rationale per sub (PRD §5.3.3)
...
### [x] A4 — Strengths/Weaknesses/Benefits — "Why Us?" (PRD §5.3.3)
...
### [x] A5 — Voice of the Customer: stakeholder link + fields (PRD §5.3.3)
...
### [x] B1 — Capabilities Alignment scoring legend + concerns row (PRD §5.3.2/§5.3.3)
...
### [x] B4 — Deal color badge descriptions (PRD §5.2, status Partial)
...
### [x] B5 — Competitor demography (PRD §5.3.2)
...
### [x] C1 — Executive Readout View (PRD §5.2, P1)
...
### [x] C2 — Role rename: analyst → standard_user (PRD §4.6)
...
| Item | Source | Size |
|---|---|---|
| [x] D1 — gateReadiness unit tests (M4) | `08-codex-action-plan.md` M4 | S |
...
| [x] D6 — `/health` endpoint performs `SELECT 1` against DB | new (found 2026-06-11: health returns ok with DB down) | S |
...
Items A1-A5, B1, B4, B5, C1, C2, D1, D6 implemented and tested (112/112 backend, 23/23 frontend, production build clean). Migration 011/012 applied to live Supabase.

- B2: verified — customer knowledge UI already exposes familiarity + 4 positioning steps. Closed by inspection.
- B3: deal review decision/decision_notes flow exists in DealDetail; Executive Readout (C1) now presents both Decision Points with outcome language. Closed.
