...
**Issue:** RLS is disabled on every table exposed to PostgREST. Confirmed a live, non-disabled legacy `anon` API key exists for this project. Anyone holding that key can call the Supabase auto-generated REST API (`https://srguiseeksrguemxdduo.supabase.co/rest/v1/<table>`) directly and read or write every row in every table — including password reset tokens and the full `users` table — completely bypassing the app's own JWT auth and Express backend. The backend itself connects via a direct `pg`/`DATABASE_URL` connection and does not use the Supabase client SDK, so the app does not need PostgREST access at all; this is a dormant but fully live attack surface, not a false positive.
**Fix:** Enable RLS on all 41 tables. Since the app never queries via PostgREST/anon key, default-deny (RLS enabled, zero policies) fully closes this without any application-code change or risk of breaking the running app. Remediation SQL was generated by the Supabase advisor and is available on request — do not apply without review, since enabling RLS without policies will block all PostgREST access to these tables (which is the intended outcome here, but confirm no other integration relies on it first).
**Fix:** See [08-codex-action-plan.md](08-codex-action-plan.md) — new item, not yet numbered; recommend adding as `C4` in next action-plan revision.
